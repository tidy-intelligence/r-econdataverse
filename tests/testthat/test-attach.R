test_that("if no packages, shows nothing", {
  expect_snapshot(cat(econdataverse_attach_message(character())))
})

test_that("message lists all core econdataverse packages", {
  local_mocked_bindings(package_version_h = function(x) "1.0.0")
  expect_snapshot(cat(econdataverse_attach_message(core)))
})
